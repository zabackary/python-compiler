...
class Plugin:
    ...
    def hook_module(self, module: ast.Module) -> ast.Module:
        """ A hook run before name translation is performed and modules are bundled """
        return module

    def hook_module_post_transform(self, module: list[ast.AST], name_generator: ModuleUniqueIdentifierGenerator) -> list[ast.AST]:
    ...
    def hook_import_resolution(self, module: str, context_path: str) -> tuple[str, str] | None:
        """ A hook run during the module resolution step. 

        This hook is called with the module to be imported's name (`module`) as well
        as the path it was imported from (`context_path`). It should return either a)
