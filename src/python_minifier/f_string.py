...
class FString(object):
    ...
    def complete_debug_specifier(self, partial_specifier_candidates, value_node):
        ...
        conversion_candidates = [
            x + conversion for x in partial_specifier_candidates]

        if value_node.format_spec is not None:
            conversion_candidates = [c + ':' + fs for c in conversion_candidates for fs in FormatSpec(
                value_node.format_spec, self.allowed_quotes).candidates()]
        ...
        for quote in self.allowed_quotes:
            ...
            for v in self.node.values:
                if is_ast_node(v, ast.Str):
                    ...
                    try:
                        completed = self.complete_debug_specifier(
                            debug_specifier_candidates, v)
                        candidates = [
                ...
                actual_candidates += ['f' + quote +
                                      x + quote for x in candidates]
...
class OuterFString(FString):
    ...
    def __str__(self):
        ...
        for candidate in candidates:
            ...
            try:
                minified_f_string = ast.parse(
                    candidate, 'python_minifier.f_string output', mode='eval').body
    ...
    def str_for(self, s, quote):
        mini_s = str(MiniString(s, quote)).replace(
            '{', '{{').replace('}', '}}')
...
class FormattedValue(ExpressionPrinter):
    ...
    def get_candidates(self):
        ...
        if self.node.format_spec is not None:
            self.printer.delimiter(':')
            self._append(FormatSpec(self.node.format_spec,
                         self.allowed_quotes).candidates())
    ...
    def visit_Str(self, node):
        self.printer.append(
            str(Str(node.s, self.allowed_quotes)), TokenTypes.NonNumberLiteral)

    def visit_Bytes(self, node):
        self.printer.append(
            str(Bytes(node.s, self.allowed_quotes)), TokenTypes.NonNumberLiteral)
    ...
    def visit_JoinedStr(self, node):
        ...
        if self.printer.previous_token in [TokenTypes.Identifier, TokenTypes.Keyword, TokenTypes.SoftKeyword]:
            self.printer.delimiter(' ')
        self._append(
            FString(node, allowed_quotes=self.allowed_quotes).candidates())
...
class Str(object):
    ...
    def __str__(self):
        ...
        if '\0' in self._s or '\\' in self._s:
            raise ValueError(
                'Impossible to represent a %r character in f-string expression part')
