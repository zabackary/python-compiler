...
from .util import arg_rename_in_place, insert, is_ast_node
...
class NameBinding(Binding):
    ...
    def should_rename(self, new_name):
        ...
        rename_cost = (old_mentions * len(self._name)) + \
            (new_mentions * len(new_name)) + additional_bytes
    ...
    def rename(self, new_name):
        ...
        for node in self.references:
            ...
            if isinstance(node, ast.Name):
                ...
                node.names = [new_name if n ==
                              self._name else n for n in node.names]
            elif isinstance(node, ast.arguments):
